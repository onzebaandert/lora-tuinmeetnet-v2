...
| **Reikwijdte** | SOIL1, SOIL2, BH1750, Waterflow, Aggregator, XIAO C3 LoRa node, Heltec LoRa zender (lnHel), Heltec LoRa ontvanger (MQTT node), Node-RED, InfluxDB en Grafana. |

Wijzigingen t.o.v. v0.5: Kas (Sensorstation) buiten scope geplaatst — hardware beschikbaar voor volgend project (veggies_and_volts); meetcyclus gewijzigd van 20 naar 10 minuten (stabiel in gebruik); v2 als afgerond systeem gemarkeerd — geen nieuwe sensor nodes gepland.
...
Buiten scope vallen mechanische constructie, waterdichte behuizing, langdurige milieuvalidatie, formele EMC-certificering en productierijpe beveiliging. Kas (Sensorstation) valt buiten scope vanaf v0.6 — hardware is vrijgemaakt voor het volgende project.
...
Alle slapende sensor nodes (SOIL1, SOIL2, BH1750, Waterflow) zijn gebaseerd op XIAO ESP32-C3 met DS3231 RTC voor phase-aligned deep-sleep wakes. Externe sensoren worden via een MOSFET alleen tijdens de meetfase van spanning voorzien.
...
- **ESP-NOW RSSI:** AGG meet de signaalsterkte per ontvangen pakket (int8, opgeslagen als low byte in `rsv0` van AggUplink141).
...
- Sensor: YF-C01 (flowswitch) + YF-201B (pulsmeting), 450 pulsen per liter (nominaal)
...
- Huidige bronnodes: SOIL1 (id=1), SOIL2 (id=2), Waterflow (id=3), BH1750 (id=4)
...
### 3.7 LoRa node C3 en lnHel (Heltec TX)
...
Slapende nodes met DS3231 gebruiken Alarm1 in interruptmodus. De INT-lijn trekt GPIO2 laag; de volgende wake wordt direct na afloop van de cyclus opnieuw geprogrammeerd.

| Node | Cyclus | Wake second | Functionele betekenis |
|---|---|---|---|
| AGG | 10 min | :05 | Vroeg wakker, luistert 30 s naar sensor nodes |
| LoRa node C3 | 10 min | :05 | Gelijk met AGG, klaar om AggUplink te ontvangen |
| SOIL1 | 10 min | :08 | Na AGG, 3 s marge voor AGG-opstart |
| BH1750 | 10 min | :08 | Zelfde venster als SOIL1 |
| SOIL2 | 10 min | :16 | Na SOIL1, voorkomt ESP-NOW botsing |
...
De 10-minutencyclus resulteert in 6 LoRa-transmissies per uur (lnHel) en een slaaptijd van ~9,5 minuten per cyclus voor alle XIAO-nodes.
...
| ec_raw | uint16 | raw | EC (SOIL1/CWT), lux (BH1750) of totaal-liters int (Waterflow) |
| case_x10 | int16 | ÷10 | Case temperature via DS3231 (°C) of sessie-vlag (Waterflow) |
...
|---|---|---|
| SOIL1 | 6V solar (10×7 cm) + LilyGo T-BAT + 18650 | — |
| SOIL2 | 6V solar + T-BAT | — |
| BH1750 | T-BAT + 18650 | Solar nog te bepalen |
| Waterflow | 6V solar + T-BAT | Buiten bij kraan |
| AGG | 6V solar + T-BAT | — |
| LoRa node C3 | Redmi powerbank | Samen met lnHel in tuin |
| lnHel (Heltec TX) | Redmi powerbank + 10W solar (buck → 5.15V) | In tuin, 6 LoRa TX per uur bij 10 min cyclus |
...
|---|---|---|
| INTERVAL_MINUTES | 10 | Cyclustijd van sensor nodes, AGG en LoRa C3 |
...
| Waterflow PULSES_PER_LITER | 450 | YF-201B nominaal |
...
- De uplinkketen levert minimaal de measurements `systeem`, `soil1`, `soil2`, `flow`, `waterflow` en `bh` in InfluxDB.
...
- Waterflow PULSES_PER_LITER staat op nominale waarde (450) — voldoende voor indicatieve meting; exacte kalibratie met bekende hoeveelheid water is optioneel.
...
## 11. Status en vervolgstappen

v2 is afgerond per 29 mei 2026. Alle vier sensornodes (SOIL1, SOIL2, BH1750, Waterflow) zijn operationeel en draaien stabiel op 10-minutencyclus.

- Batterijverloop monitoren in Grafana over meerdere weken (Vbat trends per node).
- Solar aansluitingen voor SOIL2 en BH1750 eventueel voltooien.
- v2 dient als bewezen basis voor het volgende project: **veggies_and_volts**.

...
#define SKETCH_TAG "VV_SENSORSTATION_V2"
...
struct __attribute__((packed)) Soil22 {
  ...
  int16_t  case_x10;  // DS18B20 temp × 10
  uint16_t vbat_mv;
  uint16_t ph_x10;    // niet gebruikt
  uint16_t flags;
  uint16_t rsv0;
};
#pragma pack(pop)
...
void setup(){
  ...
  ds18.begin();
  bool ds_ok = (ds18.getDeviceCount() > 0);
  float ds_temp = 0.0f;
  if (ds_ok) {
    ds18.requestTemperatures();
    delay(750);  // 12-bit conversie duurt max 750ms
    ds_temp = ds18.getTempCByIndex(0);
    if (ds_temp == DEVICE_DISCONNECTED_C) { ds_ok = false; ds_temp = 0.0f; }
  }
  Serial.printf("[DS18B20] ok=%d T=%.2f\n", ds_ok ? 1 : 0, ds_temp);
  ...
  if (esp_now_init() != ESP_OK) {
    ...
    pkt.s.case_x10   = (int16_t)lroundf(ds_temp   * 10.0f);
    ...
    for (int attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      ...
      Serial.printf("[TX] seq=%u attempt=%d tx=%d ack=%d lux=%u T_sht=%.1f H=%.1f T_ds=%.2f vbat=%.2fV flags=0x%04X\n",
                    (unsigned)pkt.s.seq, attempt, tx_ok?1:0, ack_ok?1:0,
                    (unsigned)lux_u16, sht_temp, sht_hum, ds_temp,
